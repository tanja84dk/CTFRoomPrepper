...
if [ -z "$RoomBASEDIR" ]; then
    ...
    echo "export RoomBASEDIR=\"$RoomBASEDIR\"" >>~/.bashrc
    source ~/.bashrc
fi
...
platform_Selection() {
    ...
    for choice in "${array[@]}"; do
        case "$choice" in
        [1]*)
            echo -e "Selected $choice for TryHackMe"
            PLATFORM=TryHackMe && break
            ;;
        [2]*)
            echo -e "Selected $choice for HackTheBox"
            PLATFORM=HackTheBox && break
            ;;
        [3]*)
            echo -e "Selected $choice for Vulnhub"
            PLATFORM=Vulnhub && break
            ;;
        [4]*)
            read -p 'Type in the Base Directory: ' RoomBASEDIR
            sed -i "/export RoomBASEDIR=/d" ~/.bashrc
            echo "export RoomBASEDIR=\"$RoomBASEDIR\"" >>~/.bashrc
            source ~/.bashrc
            platform_Selection
            ;;
